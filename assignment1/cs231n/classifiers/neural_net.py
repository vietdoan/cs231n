...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # classifier loss.                                                          #
    #############################################################################
    new_scores = scores - np.max(scores, 1, keepdims=True)
    exp_scores = np.exp(new_scores)
    loss = np.sum(np.log(np.sum(exp_scores, 1))) - np.sum(new_scores[range(N), y])
    ...
    #############################################################################
    temp = exp_scores / np.sum(exp_scores, 1, keepdims=True)
    temp[range(N), y] -= 1
    temp /= N
    
    dW2 = hidden_layer.T.dot(temp) + reg * W2
    db2 = np.sum(temp, 0)
    
    dhidden = np.dot(temp, W2.T)
    dhidden[hidden_layer <= 0] = 0
    
    dW1 = np.dot(X.T, dhidden) + reg * W1
    db1 = np.sum(dhidden, 0)
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      #########################################################################
      id_batch = np.random.choice(num_train, batch_size)
      X_batch = X[id_batch]
      y_batch = y[id_batch]
      #########################################################################
      #                             END OF YOUR CODE                          #
